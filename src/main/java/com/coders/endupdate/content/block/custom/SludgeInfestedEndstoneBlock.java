...
public class SludgeInfestedEndstoneBlock extends Block {
    public SludgeInfestedEndstoneBlock(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    protected void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        ...
        if (!level.getBlockState(pos.above()).is(ModBlocks.SLUDGE) && !canBeInfested(state, level, pos)) {
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context, ItemAbility itemAbility, boolean simulate) {
        ...
        if (context.getItemInHand().getItem() instanceof ShovelItem) {
            ...
            if (state.is(ModBlocks.SLUDGE_INFESTED_ENDSTONE)) {
